import { FvDownloadButton } from "../ui/fv-download-button";
import { cn } from "@/lib/utils";
import Image from "next/image";
import type { ReactNode } from "react";
import { SectionHeader } from "@/app/_components/ui/section-header";

type Feature = {
  image: string;
  imageClassName?: string;
  imageHeight: number;
  title: ReactNode;
  titleAlt: string;
  points: string[];
};

// アサインするのは CTO代行 1 名。「CTOと開発チームを提供」のような、
// 人数を誤解させる表現はここでは使わない。
const features: Feature[] = [
  {
    image: "/illust-service-overview-1.svg",
    imageClassName: "-mt-5",
    imageHeight: 178,
    title: (
      <>
        最適なCTOがリード。
        <br />
        技術だけでなく、プロダクト事業の運用経験も
      </>
    ),
    titleAlt: "最適なCTOがリード。技術だけでなく、プロダクト事業の運用経験も",
    points: [
      "元CTO等の経験者から貴社に最適な担当者を選定。",
      "プロダクトを事業として立ち上げ、市場投入し、運用してきたCTOが、MVPの先を見据えてリードします。",
    ],
  },
  {
    image: "/illust-service-overview-2.svg",
    imageHeight: 158,
    title: (
      <>
        顧問や壁打ちではない。
        <br />
        チームに入り込む内製型サービス
      </>
    ),
    titleAlt: "顧問や壁打ちではない。チームに入り込む内製型サービス",
    points: [
      "外注コードの引き継ぎも本番化も、助言で終わらせません。",
      "貴社内にCTOがいるかのように内側から技術を推進。内製ノウハウの共有も強みです。",
    ],
  },
  {
    image: "/illust-service-overview-3.svg",
    imageHeight: 163,
    title: (
      <>
        「強力な開発チームを持つ」を、
        <br />
        現実的なコストで
      </>
    ),
    titleAlt: "「強力な開発チームを持つ」を、現実的なコストで",
    points: [
      "外注に追加発注を重ねても、体制は積み上がりません。",
      "Reminusは事業の段階に合わせた費用で、体制づくりから伴走・提供します。2ヶ月トライアルも可能です。",
    ],
  },
];

export function ServiceOverview({ className }: { className?: string }) {
  return (
    <section
      id="service-overview"
      className={cn("content-auto py-24 font-sans sm:py-32", className)}
    >
      <div className="mx-auto w-[88%] max-w-[1000px] md:w-[86%]">
        <SectionHeader
          label="Feature"
          align="center"
          headingClassName="text-3xl sm:text-4xl md:text-[40px] !leading-[1.7]"
        >
          サービスの特長
        </SectionHeader>

        <div className="mt-16 flex flex-col gap-16 lg:gap-18">
          {features.map((feature, index) => (
            <div
              key={index}
              className={cn(
                "flex flex-col-reverse items-center gap-8 sm:gap-12 lg:gap-16",
                index % 2 === 1 ? "sm:flex-row-reverse" : "sm:flex-row"
              )}
            >
              {/* 旧 LP と同じ storyset 系の透過イラストなので、白枠のカードには
                  入れず素の画像として置く (枠を付けると背景が抜けて見える)。 */}
              <div className="max-w-[200px] shrink-0 md:max-w-[280px] lg:max-w-[360px]">
                <Image
                  src={feature.image}
                  width={320}
                  height={feature.imageHeight}
                  alt={feature.titleAlt}
                  className={feature.imageClassName}
                />
              </div>
              <div className="flex max-w-[600px] flex-col gap-4 lg:gap-5">
                <p className="text-lg font-bold !leading-[1.6] tracking-wide text-gray-800 sm:text-xl md:text-2xl lg:text-3xl">
                  {feature.title}
                </p>
                <p className="whitespace-pre-line text-sm !leading-[1.8] tracking-wide text-gray-800 sm:text-base md:text-lg md:!leading-[1.9]">
                  {feature.points.join("")}
                </p>
              </div>
            </div>
          ))}
        </div>

        <div className="bleed mt-20 px-4">
          <FvDownloadButton
            variant="filled"
            imageSrc="/document-cover-c-white.png"
            title="資料ダウンロード"
            subtitle="提供内容と事例を公開中"
            href="/c/download"
            className="mx-auto"
          />
        </div>
      </div>
    </section>
  );
}
